...
class medoo
{
	...
	public function query($query)
	{
		...
		$this->logSql[] = $query;
		$hasil = $this->pdo->query($query);
		if($this->pdo->errorInfo()[0]!="00000")
        	$this->logError[] = implode(" : ",$this->pdo->errorInfo());
		else {
			$this->logError[] = "Sukses";
		}
	...
	public function insertEmoji($table, $datas)
	{
		$lastId = array();
	...
	function showLogError(){
        $jml = count($this->logSql);
        echo '<pre class="well well-sm" style="word-wrap: break-word;">';
        for($n=0;$n<$jml;$n++){
            echo "[".($n+1)."] ".$this->logSql[$n]."\r\n";
            echo "[".($n+1)."] ".$this->logError[$n].'<hr class="garistipis">';
        }
        echo '</pre>';
    }
	...
	function getLogError(){
		...
		$hasil = array();
        for($n=0;$n<$jml;$n++){
			$hasil[] = array($this->logSql[$n],$this->logError[$n]);
        }
